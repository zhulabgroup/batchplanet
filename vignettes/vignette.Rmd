...
Make sure your data have the columns: `id` (unique id for each point of interest), `lon`, `lat`. You may have a character column `site` if you have concentrated coordinates at dispersed sites, like in our example. You may also a character column `group` you can use to group coordinates later on.
...
calculate_phenological_metrics_batch(dir = data_dir, v_site = "SJER", v_group = "Quercus", df_thres = df_thres, min_days = 20, check_seaonality = F, num_cores = 1)
```
