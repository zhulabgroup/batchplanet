...
- If you want to download PlanetScope imagery over multiple locations and over a long time (optional but recommended).
...
For demonstration purpose, we are going to set the data directory to the package's extdata folder, but you should use `set_data_directory()` as above when you carry our your own analysis.
```{r}
...
*The function runs correctly itself, but the error "could not find function "visualize_true_color_imagery_batch" occurs in this vignette.*
...
calculate_phenological_metrics_batch(dir = dir_data, v_site = "SJER", v_group = "Quercus", df_thres = df_thres, var_index = "evi", min_days = 20, check_seasonality = F, extend_to_previous_year = 275, extend_to_next_year = 90, num_cores = 3)
```
...
df_doy_sample <- read_data_product(dir = dir_data, v_site = "SJER", v_group = "Quercus", product_type = "doy") %>% filter(id %in% v_id)
df_evi_sample <- read_data_product(dir = dir_data, v_site = "SJER", v_group = "Quercus", product_type = "clean") %>% filter(id %in% v_id)
