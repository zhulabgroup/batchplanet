---
title: "NEON Site Analysis Workflow"
...
This workflow demonstrates the analysis capabilities of the batchplanet package using NEON site data. We'll explore various functions for processing and visualizing coordinate based data.
...
df_coords <- read_csv("inst/extdata/example_neon_coordinates.csv", show_col_types = FALSE)

# Basic coordinate visualization using vis_coords.R
vis_coords(df_coords) # Multiple sites across continental US
vis_coords(df_coords %>% filter(site == "HARV")) # Zoom in to one site HARV
```

Make sure your data have the columns: lon, lat, site.

## 2 Download PS data

### 2.1 Set downloading parameters
Replace API key with your own here.
```{r}
setting <- set_ps(api_key = "[your API key]", harmonized = T)
dir_data <- "inst/extdata/"
```

### 2.2 Order
We are ordering images from two sites in one year as an example. Order IDs saved as rds.
```{r, eval = F}
order_satellite_batch(dir = dir_data, df_locations = df_coords, v_site = c("HARV", "SJER"), v_year = 2025, setting = setting)
```
...
```{r, eval = F}
down_satellite_batch(dir = dir_data, v_site = c("HARV", "SJER"), v_year = 2025, setting = setting, num_cores = 3)
```
