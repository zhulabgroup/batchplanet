...
df_index <- read_evi(dir = "alldata/juniper_ashei/PSdata/evi/", v_site = NULL, v_taxa = "Juniper ashei") %>%
  # select(-site) %>%
  mutate(g2b = green / blue) %>%
  mutate(yellow = (red + green) / blue) %>%
  mutate(ebi = yellow / (green / blue) * (red - blue + 1)) %>% 
  rowwise() %>% 
  mutate(orange = percentage_green_to_orange(c(red, green, blue), thres = 0)) %>% 
  mutate(hex = rgb_to_hex(c(red, green, blue)) ) %>% 
  ungroup() 
...
Calculate and visualize green to blue ratio.
```{r}
vis_ts(df_index, var = "g2b", ylab = "G/B", smooth = T)
```
...
```{r, eval = F}
vis_ts(df_index_version, n_id = 3, var = "g2b", ylab = "G/B", smooth = T, facet = "version")
...
```{r, eval = F}
vis_ts(df_index_version, v_year = 2021:2024, n_id = 50, var = "evi", ylab = "EVI", smooth = T, facet = "group")
vis_ts(df_index_version, v_year = 2021:2024, n_id = 50, var = "g2b", ylab = "G/B", smooth = T, facet = "group")
vis_ts(df_index_version, v_year = 2021:2024, n_id = 50, var = "yellow", ylab = "yellow", smooth = T, facet = "group")
```
a