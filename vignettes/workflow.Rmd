...
```{r}
visualize_coordinates(df_coordinates_urban %>% dplyr::sample_n(2000)) # Multiple sites across continental US
visualize_coordinates(df_coordinates_urban %>% dplyr::filter(site == "NY") %>% dplyr::sample_n(2000)) # Zoom in to one site NY
```
...
```{r, eval = F}
clean_planetscope_time_series_batch(dir = dir_data_NEON, v_site = c("HARV", "SJER"), v_group = c("Acer", "Quercus"), num_cores = 3, calculate_evi = T)
```
